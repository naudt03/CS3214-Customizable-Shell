...
handle_child_status(pid_t pid, int status)
{
     ...
     */
        ...
        struct job *job = find_job_by_pid(pid);  // Implement this helper function based on your job management system

    if (job == NULL) {
        return;  // No job found for this pid
    }

    if (WIFSTOPPED(status)) {
        job->status = STOPPED;
        tcgetattr(STDIN_FILENO, &job->saved_tty_state);  // Save terminal state
    } else if (WIFEXITED(status) || WIFSIGNALED(status)) {
        job->num_processes_alive--;
        if (job->num_processes_alive == 0) {
            job->status = NEEDSTERMINAL;  // Update status to reflect no more running processes
        }
    }
